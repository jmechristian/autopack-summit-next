...
import { sendGuardianWelcome, sendSurgereWelcome } from '../../../../util/api';
...
const Page = () => {
  ...
  useEffect(() => {
    const approveUser = async () => {
      ...
      try {
        await sendSurgereWelcome(router.query.email);

        const status = await API.graphql({
          query: updateSurgereForm,
          variables: { input: { id: router.query.id, approved: true } },
        });

        if (status.data.updateSurgereForm.id) {
